...
class Information:
    ...
    def check_empty_information(self):
        if not self.get_request_title():
            ...
            talk("Quel est l'objet de votre demande:")
        ...
        while not self.get_place():
            print("Quel est le lieu de l'incident?")
            talk("Quel est le lieu de l'incident?")
            inp = listen()
            self.PLace(inp)
        if not self.get_category():
            print("quelle est la categorie de votre demande :(IT, PROPRETE, BADGE, CLIMATISATION ou AUTRE)")
            talk("quelle est la categorie de votre demande :(IT, PROPRETE, BADGE, CLIMATISATION ou AUTRE)")
        ...
        if not self.get_comment():
            print("Un commentaire a ajouter? :")
            talk("Un commentaire a ajouter? :")
    ...
    def Identify(self):
        ...
        talk('Votre nom complet:')
        name = listen()

        print('Votre numero de téléphone:')
...
if __name__ == '__main__':
    ...
    while True:
        print("En quoi pouvons nous vous aider ? ")
        talk("En quoi pouvons nous vous aider ? ")
        request = listen()
        if request == "q":
            break
        ...
        print("Merci votre demande sera étudiée par l'equipe informatique. Vous aurez un retour d'ici 48h ")
        # print('Entities', [(ent.text, ent.label_) for ent in doc.ents])
